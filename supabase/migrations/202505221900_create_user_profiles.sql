...
-- Création du type d'énumération pour les rôles utilisateur s'il n'existe pas déjà
DO $$
BEGIN
...
BEGIN
    ...
    ELSE
        ...
        IF NOT EXISTS (
            ...
            ALTER TABLE profiles ADD COLUMN user_role user_role DEFAULT 'user'::user_role;
        END IF;

        -- Vérifier si la colonne social_links existe
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.columns
        ...
        -- Ajouter d'autres vérifications pour les colonnes importantes si nécessaire
    END IF;
END$$;
